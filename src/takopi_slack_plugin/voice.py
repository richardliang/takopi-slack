...
async def transcribe_voice(
    *,
    ...
    if not enabled:
        ...
        return None
    if file.size is not None and max_bytes is not None and file.size > max_bytes:
        await reply(text="voice file is too large to transcribe.")
        return None
    url = file.url_private_download or file.url_private
    if url is None:
        await reply(text="voice file is missing a download url.")
        return None
